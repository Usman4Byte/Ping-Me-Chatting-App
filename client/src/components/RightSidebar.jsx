  ...
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-auto hide-scrollbar ${selectedUser ? 'max-md:hidden' : ''}`}>
      ...
      <div className='px-5 text-xs'>
        <p>Media</p>
        <div className='mt-5 max-h-[200px] overflow-y-auto hide-scrollbar grid grid-cols-2 gap-4 opacity-80'>
          {msgImages.map((url, index) => (
            <div key={index} onClick={() => window.open(url)} className='cursor-pointer rounded overflow-hidden'>
              <img src={url} className='rounded-md h-24 w-full object-cover' alt={`media-${index}`} />
            </div>
          ))}
        </div>
      </div>
